...
export async function PATCH(
  ...
  try {
    ...
    // Verificamos que el campo 'seguimiento' esté definido
    if (typeof data.seguimiento !== 'boolean') {
      return NextResponse.json(
        { error: "El campo 'seguimiento' es requerido y debe ser un booleano" },
        { status: 400 }
      )
    }
    ...
    // Actualizar el campo de seguimiento
    try {
      const actualizarDetalle = await prisma.gastoDetalle.update({
        where: { id: detalleId },
        data: {
      ...
      return NextResponse.json({
        success: true,
        message: `Seguimiento ${data.seguimiento ? 'activado' : 'desactivado'} correctamente`,
