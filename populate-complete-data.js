// Script para generar datos de prueba para la aplicación de gastos
// Este script:
// 1. Elimina todos los datos existentes excepto los usuarios
// 2. Crea un grupo asociado al usuario principal
// 3. Crea algunos gastos asociados a un usuario ficticio
// 4. Genera datos completos de transacciones, gastos recurrentes, presupuestos y financiaciones
...
async function populateDatabase() {
    ...
    try {
      ...
      if (categoria) {
        await prisma.presupuesto.create({
          data: {
            nombre: presupuesto.nombre,  // Añadido el campo nombre aquí
  ...
  for (const { mes, año } of meses) {
    ...
    try {
      await prisma.gasto.create({
        data: {
          ...
          userId: usuario.id,
          // Sin categoría específica para ingresos
        }
      });
    ...
    for (const [categoria, porcentaje] of Object.entries(DISTRIBUCION_GASTOS)) {
      ...
      for (let i = 0; i < numTransacciones; i++) {
        ...
        const metodosPago = ['tarjeta', 'efectivo', 'transferencia', 'móvil'];
        const metodoPagoElegido = metodosPago[Math.floor(Math.random() * metodosPago.length)];
        ...
        if (eventoAleatorio.evento === 'compra-tecnologia') {
          ...
          await prisma.financiacion.create({
            data: {
              ...
              cuotasRestantes: 6  // Añadido el campo cuotasRestantes aquí
            }
          });
  ...
  // 8. CREAR USUARIO FICTICIO CON ALGUNAS TRANSACCIONES
